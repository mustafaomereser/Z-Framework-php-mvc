...
class DB
{
    ...
    public function paginate($per_page_count = 20, $page_request_name = 'page', $class = false)
    {
        ...
        $return = [
            'items'          => $row_count ? array_slice($rows, $start_count, $per_page_count, true) : [],
    ...
    private function getLimit()
    {
        ...
        switch ($this->driver) {
            ...
            case 'sqlsrv':
                if ($limit) {
                    if (!is_null($limit[1])) {
                        if (!isset($this->buildQuery['orderBy'])) $this->buildQuery['orderBy'] = ['id' => 'ASC'];
                        return " OFFSET " . (is_null($limit[1]) ? 0 : $limit[0]) . " ROWS FETCH NEXT " . (is_null($limit[1]) ? $limit[0] : $limit[1]) . " ROWS ONLY";
                    } else {
                        $this->select("TOP (" . $limit[0] . ") " . $this->buildQuery['select']);
                    }
                }
